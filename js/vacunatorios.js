class VacunatoriosMap {
    constructor() {
        ...
        this.CACHE_KEY = 'vacunatorios_cache_v2';
        this.COORDINATES_CACHE_KEY = 'coordinates_cache_v2';
    ...
    async init() {
        console.log('Iniciando mapa mejorado...');
        try {
    ...
    async loadVacunatoriosWithCoordinates() {
        console.log('Cargando vacunatorios con coordenadas...');
        ...
        try {
            console.log('Intentando cargar archivo de coordenadas...');
            ...
            if (coordinatesResponse.ok) {
                ...
                console.log(`Coordenadas cargadas: ${this.coordinatesData.length} ubicaciones`);
            ...
            console.log('Archivo de coordenadas no disponible, usando método tradicional');
        }

        console.log('Fallback: usando geocodificación en tiempo real...');
    ...
    async loadVacunatoriosOriginal() {
        ...
        if (cachedData) {
            console.log('Usando datos CSV en caché');
    ...
    displayAllMarkers() {
        ...
        this.coordinatesData.forEach(vacunatorio => {
        ...
        console.log(`${markersAdded} marcadores agregados al mapa`);
    ...
    addMarkerFromCoordinates(vacunatorio) {
        ...
        if (isNaN(lat) || isNaN(lng)) {
            console.warn('Coordenadas inválidas para:', vacunatorio.nombre);
            return;
        }
    ...
    async filterVacunatorios() {
        ...
        const filteredVacunatorios = dataToFilter.filter(v => {
            ...
            const domicilio = v.domicilio || v.Domicilio || '';
            const localidad = v.localidad || v.Localidad || '';

            const matchesSearch = !searchText ||
                nombre.toLowerCase().includes(searchText) ||
                domicilio.toLowerCase().includes(searchText) ||
                localidad.toLowerCase().includes(searchText);
        ...
        if (this.coordinatesData.length > 0) {
            filteredVacunatorios.forEach(vacunatorio => {
                if (vacunatorio.lat && vacunatorio.lng) {
                    this.addMarkerFromCoordinates(vacunatorio);
                }
            });
                    ...
                    <div class="popup-services">
                        ...
                        <div class="services-grid">
                            <div class="service-item ${apVacuna === 'Si' ? 'available' : 'unavailable'}">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="${apVacuna === 'Si' ? '#27ae60' : '#e74c3c'}">
                                    <path d="M9,20.42L2.79,14.21L5.62,11.38L9,14.77L18.88,4.88L21.71,7.71L9,20.42Z"/>
                                </svg>
                                <span>Vacunas generales</span>
                            </div>
                            <div class="service-item ${apVacunaMenor === 'Si' ? 'available' : 'unavailable'}">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="${apVacunaMenor === 'Si' ? '#27ae60' : '#e74c3c'}">
                                    <path d="M9,20.42L2.79,14.21L5.62,11.38L9,14.77L18.88,4.88L21.71,7.71L9,20.42Z"/>
                                </svg>
    ...
    updateResultsList(vacunatorios) {
        ...
        vacunatorios.slice(0, 50).forEach((vacunatorio, index) => {
            ...
            const tipo = vacunatorio.tipo || vacunatorio.Tipo || 'Centro de Salud';
            const domicilio = vacunatorio.domicilio || vacunatorio.Domicilio || 'Sin dirección';
            const localidad = vacunatorio.localidad || vacunatorio.Localidad || 'Sin localidad';
            const provincia = vacunatorio.provincia || vacunatorio.Provincia || 'Sin provincia';
            const telefono = vacunatorio.telefono || vacunatorio.Telefono || '';
            const apVacuna = vacunatorio.apVacuna || vacunatorio['Ap.Vacuna'] || 'No';
            const apVacunaMenor = vacunatorio.apVacunaMenor || vacunatorio['Ap.Vacuna Menor'] || 'No';
    ...
    saveCoordinatesToCache(data) {
        ...
        try {
            ...
            console.log('Coordenadas guardadas en caché');
        } catch (error) {
            console.error('Error guardando coordenadas en caché:', error);
        }
    }
    ...
    getCoordinatesFromCache() {
        try {
            ...
            if (isExpired) {
                console.log('Caché de coordenadas expirado');
                localStorage.removeItem(this.COORDINATES_CACHE_KEY);
                return null;
            }

            console.log('Coordenadas recuperadas de caché');
            return data;
        } catch (error) {
            console.error('Error recuperando coordenadas de caché:', error);
            return null;
        }
    }
